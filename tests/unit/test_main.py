...
async def test_status_endpoint():
    ...
    with patch("src.service.status.StatusService.handle_request", new_callable=AsyncMock) as mock_handle:
...
async def test_startup_event_success():
    ...
    with patch("src.service.user.UserService.handle_request", new_callable=AsyncMock) as mock_user_handle, \
         patch("src.config.BodySettings.load", return_value={"database": "test_db", "username": "test_user", "password": "test_password"}):
